...
%vertebra to which landmark you want to calculate angle to:
LMnmbr = 6;

% replace with the path of your folder (control, preop or postop)
path = 'C:\Users\timhe\Desktop\postop\';



%%
% get information about all files and folders in the directory
...
for m=1:length(folder)
    ...
    if length(files)==4
        ...
        % 3d this one is having point 400 hundred straight above point 4

        for i = 1:length(ribs)
            
            pos4=[ribs{i}(4,1),ribs{i}(4,2),ribs{i}(4,3)];
            posonrib=[ribs{i}(LMnmbr,1),ribs{i}(LMnmbr,2),ribs{i}(LMnmbr,3)];
            
            if i == 1
                pos4up = pos4;
                pos4down = [ribs{i+1}(4,1),ribs{i+1}(4,2),ribs{i+1}(4,3)];
            elseif i == 4
                pos4up = [ribs{i-1}(4,1),ribs{i-1}(4,2),ribs{i-1}(4,3)];
                pos4down = pos4;
            else
                pos4up=[ribs{i-1}(4,1),ribs{i-1}(4,2),ribs{i-1}(4,3)];
                pos4down=[ribs{i+1}(4,1),ribs{i+1}(4,2),ribs{i+1}(4,3)];
        ...
        % 2d
        % this angle is between the point 400 and 4 and the point 6 taking into
        % account the z-x movement, keeping the posterior-anterioir side fixed
        for i = 1:length(ribs)
            pos4=[ribs{i}(4,1), 1 , ribs{i}(4,3)];
            posonrib=[ribs{i}(LMnmbr,1), 1 , ribs{i}(LMnmbr,3)];
            if i == 1
                pos4up = pos4;
                pos4down = [ribs{i+1}(4,1),1,ribs{i+1}(4,3)];
            elseif i == 4
                pos4up = [ribs{i-1}(4,1),1,ribs{i-1}(4,3)];
                pos4down = pos4;
            else
                pos4up=[ribs{i-1}(4,1),1,ribs{i-1}(4,3)];
                pos4down=[ribs{i+1}(4,1),1,ribs{i+1}(4,3)];
